---
title: "Final evaluation of total aphids (22.06.2018)"
...
dat_raw <- read.csv2("Bonitur_2final.csv", header=TRUE, strip.white=TRUE)
...
# Im Datensatz fehlt die Variable pnr. Hab die jetzt aus Bonitur 1 genommen und 
# an diesen Datensatz dran gebaut. Bitte noch mal im original-Datensatz ändern!!!
# dat_raw$Pnr <- factor(read.csv2("Bonitur_1eng.csv", header=TRUE, strip.white=TRUE)$Pnr)
...
datsum <- dat_raw %>% 
        ...
        summarize(aphids_sum=sum(aphtot),
                  n=n(),
...
* _aphids_sum:_  sum of aphids per location in each plot
...
since the p-value is 0.025, but no location effect (inpl) nor a 
